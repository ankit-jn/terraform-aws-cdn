...
resource aws_cloudfront_response_headers_policy "this" {
    ...
    dynamic "cors_config" {
        for_each = try(each.value.configure_cors, false) ? [1] : 0
        ...
        content {
            ...
            dynamic "access_control_expose_headers" {
                for_each = length(try(each.value.exposed_headers, [])) > 0 ? [1] : 0
    ...
    dynamic "custom_headers_config" {
        for_each = length(try(each.value.custom_headers, [])) > 0 ? [1] : 0
    ...
    dynamic "server_timing_headers_config" {
        for_each = try(each.value.enable_server_timing_header, false) ? [1] : 0
